...
``` go
func xxxx(){
  respData []map[string]any  // 这里是map切片
  // ...
	var metadata mapstructure.Metadata
	KsDayCreativeReportList := make([]*response.KsCreativeDayRspDataItem, 0)
	// tag: 注意这里由于数据是从csv文件获取，所以值都是字符串，需要自动推断（启用弱类型转换）
	// err := mapstructure.WeakDecodeMetadata(respData, &KsDayCreativeReportList, &metadata)
	// 创建一个新的解码器配置
	decoderConfig := &mapstructure.DecoderConfig{
		WeaklyTypedInput: true, // 启用弱类型转换
		Result:           &KsDayCreativeReportList,
		Metadata:         &metadata,
		// DecodeHook: customHook, // 设置自定义解码钩子
	}
	decoder, err := mapstructure.NewDecoder(decoderConfig)
	if err != nil {
		log.Context(ctx).Infof("[CreativeDayReportHistory][%d]#advertiserId:%s, NewDecoder err:%+v", cate, advertiserId, err)
		return err
	}
	// 使用解码器进行解码
	err = decoder.Decode(&respData)
	if err != nil {
		log.Context(ctx).Warnf("[CreativeDayReportHistory][%d]#advertiserId:%s, details转换结构体异常WeakDecode#%+v", cate, advertiserId, err)
		return err
	}
...
func customHook(f reflect.Type, t reflect.Type, data interface{}) (interface{}, error) {
	// f源 t目标
	// util.GetInterfaceToInt()
	switch t.Kind() {
	case reflect.Int:
		tInt, _ := strconv.Atoi(data.(string))
		return tInt, nil
	case reflect.Int64:
...
// 如果是函数内要使用 匿名函数
toPersonLocationHookFunc := func() mapstructure.DecodeHookFunc {
		return func(f reflect.Type, t reflect.Type, data interface{}) (interface{}, error) {
      //....
    }
}

respData Person  // 也可以直说对象

// 创建解码器并设置自定义钩子函数
decoder, err := mapstructure.NewDecoder(
  &mapstructure.DecoderConfig{
    DecodeHook: toPersonLocationHookFunc(),
    Result:     &respData, // 目标数据
  }
)
if err != nil {
  panic(err)
