...
## 2.0 int64, float64精度丢失 超出int范围时
...
```
## 3.0 float32位 和float64位 json字符串数值精度解析问题问题
``` go
...
## 4.0 json字符串里的字段大小写和json数组（对象数组）
- josn标签里的大小写 不影响json字符串里字段的解析（这里大小写不影响）
...
// err = json.Unmarshal([]byte(body), &ttRespInfo) // tag: q其实有int64时
decoder := json.NewDecoder(strings.NewReader(body))
decoder.UseNumber()
err = decoder.Decode(&ttRespInfo)
if err != nil {
	log.Context(ctx).Warnf("json unmarshal err : %+v", err)
	return "", err
}
...
```
# <font color="red">mapstructure性能问题</font>
之所以在 jrpc 请求时使用 mapstructure 进行解析，主要是因为 jrpc 基本上都是请求 php 服务，php响应都比较随意，同一个接口的同一个响应字段可能是 int 或 string，这使得用 原生 json 无法将请求响应反序列化为结构体。
https://o15vj1m4ie.feishu.cn/wiki/IRfwwLVALi7HMQkA4U5cgWOqnac?open_in_browser=true 
todo: 解析一下

![alt text](image6.png)
- 红色部分，可以用sonic库替换原生json库来解决 sonic字节包
- 蓝色部分 没必要用 maptostructure 这个性能那么差的库来解析 消息能够直接使用 json 反序列化，理论上不确定的字段类型用 flexjson 就行了（频率多就改，尤其是消息队列解析消费）

# php接口返回的结果json字符串解析
- go调用php的时候(rpc),就是被调用的接口（php提供），<font color="red">正常情况下会返回一个数组，在没有数据情况下会返回空数组[]</font>等问题 考虑使用自己封装flexjson包（内部使用了sonic）

# json Unmarshal性能提升
  
- 问题：最近收到内存告警，一个平常极其稳定的用于数据采集的服务出现OOM，经过排查并不是内存泄露，就是单纯的大数据量情况下反序列化内存不够用
- 原因：
...
- 解决方案
1. 延迟解析
   - golang关于较大规模且复杂结构的json 反序列化Q时，可以考虑分两步的方式：先只反序列化key，然后再按需取key对应的结构再进行 反序列化。这样可以避免一次性反序列化完整的结构导致性能出现巨大损耗。
   - json.RawMessage 是 Go 标准库提供的一种类型，它允许你延迟解析 JSON 数据的某些部分，直到真正需要时再解析。
2. 使用sonic包 github.com/bytedance/sonic
