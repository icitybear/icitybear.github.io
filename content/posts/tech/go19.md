...
## 相关函数
**[源码解析](https://blog.csdn.net/roddehope/article/details/118660460)**
...
- CancelFunc 是一个函数类型，调用它时会撤销对应的子 Context 对象, <font color="red">思考主函数里 defer cancelFunc()?</font>
- 当满足某种条件时，我们可以通过调用该函数CancelFunc结束所有子协程的运行，主协程在接收到信号后可以继续往后执行。
- cxt.Done() 方法返回一个通道，该通道会在调用 cancelFunc 函数时关闭，或者在父 context 撤销时也会被关闭。 等待阻塞用
