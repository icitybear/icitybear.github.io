...
热更新：log打得太多了可以动态调高日志等级，业务逻辑参数变化，甚至某个功能模块的开关等都可以动态调整
``` go
package main  
import ( 
  "encoding/json" 
...
var ( 
  config   *Config // 全局配置
  configLock = new(sync.RWMutex)  // 读写锁
) 
  
func loadConfig() bool { 
  f, err := ioutil.ReadFile("config.json")  // 读取配置文件
  if err != nil { 
...
func init() { 
  ...
  signal.Notify(s, syscall.SIGUSR1) // 监听信号量signal.Notify

  go func() { 
    for { 
      <-s 
	  rgx := loadConfig()
      log.Println("Reloaded config: ", rgx) 
    } 
  }() 
} 
