...
3. 不存在就新建，考虑直接使用 FirstOrCreate 直接创建记录了
``` go
...
4. 初始化方法FirstOrInit, 配合Attrs方法和Assign方法。 必须是struct（map不行）
...
-  结构体里的条件字段 []clause.Column{{Name: "id"},{Name: "xxx"}}, 必须包含唯一键
   -  <font color="red">包含了唯一键字段（ID 或其他你定义的联合唯一键）</font>, 如果设置非主键字段就会每次都是新增
-  结构体里的更新字段 clause.AssignmentColumns([]string{"age", "xxx"})
- <font color="red">可以处理批量数据</font>
- <font color="red">通过将 DoNothing: true 选项传递给 OnConflict 结构体，可以防止更新现有记录，实现找到记录时不更新而直接插入新记录的效果。</font>
  - 如果忘记填写 []clause.Column 会导致报错 Error 1869 (HY000): Auto-increment value in UPDATE conflicts with internally generated values

- 确保在遇到重复键时，Gorm 不会尝试更新自增字段，从而避免了错误 db.Clauses(clause.Insert{Modifier: "IGNORE"}).Create(&model)
目的是在执行插入操作时，如果遇到唯一键冲突（比如主键或唯一索引冲突），则忽略这条记录，不执行插入操作。这通常用于处理可能存在重复数据的情况，特别是在批量插入数据时，我们不希望因为个别数据的冲突而影响整个插入操作。
...
3. take不会进行排序；检索单个对象，LIMIT 1;not found会报错
