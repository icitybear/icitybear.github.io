...
``` 
## 3. Metrics 的自定义使用(中间件的处理)
-  http服务和grpc服务使用metrics中间件， 该中间件也针对自定义的Meterics进行字段处理
- metrics中间件的Server方法 生成/返回 middleware.Handler 
  - 选项参数/方法 metrics.WithSeconds和metrics.WithRequests
  - prom.NewCounter 调用的是kratos封装的prometheus数据结构:prometheus.NewCounterVec
  - prom.NewHistogram 调用的是kratos封装的prometheus数据结构:prometheus.NewHistogramVec
``` go
package server

import (
	extLog "git.internal.xxx.cn/go-modules/kratos-extends/log"
	"github.com/go-kratos/kratos/v2/middleware/metrics" // kratos的指标中间件 
	"github.com/prometheus/client_golang/prometheus" // 官包的采集器
	"time"

	extEncoder "git.internal.xxx.cn/go-modules/kratos-extends/encoder"
	extFilter "git.internal.xxx.cn/go-modules/kratos-extends/filter"
	extAC "git.internal.xxx.cn/go-modules/kratos-extends/middleware/accesslog"
	extBp "git.internal.xxx.cn/go-modules/kratos-extends/middleware/baseparams"
...
func newHTTPServer(appName string, c *conf.Server) *http.Server {
	...
	return http.NewServer(httpOpts...) // 启动http服务 会注册多个路由
}

```


prometheus.MustRegister 注册的 一般写在init()方法里，多个map的可以统一写在一个文件里，参照job
1. 不使用中间件 直接启用/metrics接口服务
``` go
package server

func New(appName string, c *conf.Server) (*kratos.App, func(), error) {
	// 启动GRPC，HTTP Server服务端口
	grpcSrv := newGRPCServer(appName, c)
...
import (
	...
	"git.internal.xxx.cn/go-modules/kratos-extends/httpclient"
	prom "github.com/go-kratos/kratos/contrib/metrics/prometheus/v2"
	"github.com/go-kratos/kratos/v2/metrics"
	"github.com/prometheus/client_golang/prometheus"
)

var promClientSec metrics.Observer
var promClientCount metrics.Counter

func init() {
	// 是否动态区分系统 暂时先区分媒体
	metricSeconds := prometheus.NewHistogramVec(prometheus.HistogramOpts{
		Namespace: "tuiguang",
		Subsystem: "marketing_api",
		Name:      "http_request_sec",
		Help:      "the third api http desc cost",
		Buckets:   []float64{0.005, 0.01, 0.025, 0.05, 0.1, 0.250, 0.5, 1, 2.5, 5, 10},
	}, []string{"method", "media_code", "uri", "http_code", "system"})

	metricRequests := prometheus.NewCounterVec(prometheus.CounterOpts{
		Namespace: "tuiguang",
		Subsystem: "marketing_api",
		Name:      "http_request_total",
		Help:      "the third api http desc count",
	}, []string{"method", "media_code", "uri", "http_code", "system"})

	prometheus.MustRegister(metricSeconds, metricRequests) // 注册

	promClientSec = prom.NewHistogram(metricSeconds) // 记得赋值给变量
	promClientCount = prom.NewCounter(metricRequests)
}

/*--------*/
	labelValues := []string{method, mediaCode, path, strconv.Itoa(resp.Status), reqSystem}
	// 请求完毕进行监控
	if promClientSec != nil {
		promClientSec.With(labelValues...).Observe(time.Since(t1).Seconds())
	}

	if promClientCount != nil {
		promClientCount.With(labelValues...).Inc()
	}
/*--------*/
``` 

- 例子2: mp-admp-material 单pod指标采集，15s，200M+ , 采集是放一起的，统一一个mp-admp，这个 mp-admp-material  量大，影响了整个采集
...
- 中间件 bi
``` go
// pulsar队列积压 因为pulsar有很多分区
sum(pulsar_subscription_back_log{cluster="xxx-online-pulsar", namespace="middle_tenant/mp_ns", topic=~"persistent://middle_tenant/mp_ns/admp_material_supplement-partition-.+"})
```
...
sum(sum_over_time(ck_monitor_xxx_system_requests_error_count_for_5m_rules_admp{system=~"admp|ammp|admprta|agmp|mp-admp-data|mp-admp-data-job|mp-smart-admp|mp-smart-admp-job"}[5m])) by(system)  > 50
// 每分钟Nginx返回码非200大于50
sum by(system,retCode) (ck_monitor_xxx_system_nginx_requests_code_count{system=~".*admp.*|agmp|ammp", retCode!="200"})  > 50
// 【竞价数量变化】每分钟竞价数量同比昨天上涨30% 多个标签组合分组
SUM(increase(mp_admp_rta_service_rta_bid_info_count{}[1m])) by(media_code, rta_id) / SUM(increase(mp_admp_rta_service_rta_bid_info_count{}[1m]offset 1d)) by(media_code,rta_id) > 1.3

// 推广服务pod发生了重启
ack:pod:restart{pod=~".*admp.*",pod!~".*drama.*"} >= 1
// 网关请求投放系统参数错误拦截 ADMP服务端
sum(increase(param_err_count{system=~".*admp.*|agmp|ammp"}[1m])) by (system)> 50
// 投放系统api网关接口未找到告警
...
``` go
// 2个指标
tuiguang_marketing_api_http_request_total  prometheus.HistogramOpts
tuiguang_marketing_api_http_request_sec prometheus.CounterOpts
// kubernetes_pod_label_name="mp-admp-rta"
sum(rate(tuiguang_marketing_api_http_request_total{}[$__rate_interval])) by (media_code) // 整体qps

sum(rate(tuiguang_marketing_api_http_request_total{media_code="toutiao"}[$__rate_interval])) by (uri)

rate(tuiguang_marketing_api_http_request_sec_sum{media_code="toutiao"}[$__rate_interval]) / rate(tuiguang_marketing_api_http_request_sec_count{media_code="toutiao"}[$__rate_interval])

histogram_quantile(0.95, sum(rate(tuiguang_marketing_api_http_request_sec_bucket{media_code="toutiao"}[$__rate_interval])) by (uri, le))
...
1. $__rate_interval 是一个 内置时间间隔变量，专门用于 Prometheus 数据源的 rate() 或 irate() 函数，它能动态计算适合当前时间范围的间隔值，避免出现 "单调性错误"（monotonicity errors）和查询过载问题。
  - 根据当前面板的 时间范围跨度 自动调整 rate() 的窗口大小
  - 公式：$__rate_interval = max($__interval + scrape_interval, 4 * scrape_interval)
    - $__interval：Grafana 图表的分辨率（取决于面板时间范围和像素宽度）
    - scrape_interval：Prometheus 抓取间隔（默认 15s-1m）
    - rate(metric[5m]) 问题 小时间范围可能数据不足（告警sql使用）,如果时间太小 面版报错"range vector selector needs at least 2 samples"
    - rate(metric[$__rate_interval]) 动态规避错误
    - $__range：当前面板的 总时间范围（如 6h）, $__rate_interval：用于 rate() 的 滑动窗口大小
  
2. Counter 类型指标（必须配合 rate()）直接使用 sum(http_requests_total)（Counter 单调递增无意义）
	- sum(increase(mp_admp_material_job_material_crawl_count{}[1d]))

3. histogram 类型指标

- histogram_quantile(φ float, b instant-vector) 函数用于计算直方图指标的分位数。它需要两个参数：
  - φ：分位数，介于 0 和 1 之间（例如，0.95 表示第95百分位数）。
  - b：一组直方图桶的瞬时向量，这些桶必须具有 le 标签（表示桶的边界）。

- le 小于或等于）的缩写，用于表示直方图桶（bucket）的上限边界值，使 histogram_quantile() 能够重建值的分布并计算精确分位数
``` go	
	bucket{le="0.1"} → 20 次请求 (≤0.1s)
	bucket{le="0.5"} → 50 次请求 (包含前面的20次 ≤0.1s + 30次 0.1-0.5s)
	bucket{le="1.0"} → 70 次请求 (包含前面所有)
	bucket{le="+Inf"} → 75 次请求 (总请求数)
```
