...
1. 跨结构体字段复制
   ...
   - 如果需要不同类型之间的转换，必须使用 CopyWithOption 并配置 Converters（自定义类型转换器）
...
3. 切片与Map处理
   ...
   - 当目标是 slice 时，必须传 slice 的指针 &records，这样 copier 才能通过反射修改原始 slice 的底层数组。
...
// string → int 类型不匹配时，字段被跳过，保持零值。这是 copier 所有版本的默认行为。
func TestCopyDifferentFieldType(t *testing.T) {
	...
	ts := &TypeStruct1{Field1: 111, Field2: 222}
	ts2 := &TypeStruct2{}
	copier.Copy(ts2, ts)
	fmt.Println(ts2) // &{ }
	// Field1: string -> int, 期望结果是 0（零值，即跳过）
	// Field2: string -> string, 正常复制

	// 如果需要不同类型之间的转换，必须使用 CopyWithOption 并配置 Converters（自定义类型转换器）。
	copier.CopyWithOption(ts2, ts, copier.Option{
		Converters: []copier.TypeConverter{
			{
				SrcType: copier.Float64,
				DstType: copier.String,
				Fn: func(src interface{}) (interface{}, error) {
					return fmt.Sprintf("%.2f", src.(float64)), nil
				},
			},
		},
	})
	fmt.Println(ts2) // &{111.00 222.00}
...
func TestCopySlice(t *testing.T) {
	...
	ts := make([]*TypeStruct1, 0)
	ts = append(ts, &TypeStruct1{Field1: 333, Field2: 0})
	ts2 := make([]*TypeStruct2, 0)
...
# gconv 类型转换与数据解析工具
- https://github.com/gogf/gf/tree/master/util/gconv 源码地址 goframe框架的
- imporot "github.com/gogf/gf/v2/util/gconv"
