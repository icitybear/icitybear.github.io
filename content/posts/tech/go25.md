...
- <font color="red">在操作系统中，锁、条件变量和信号量都是用来实现线程同步与互斥的机制</font>
...
## 条件变量 sync.Cond
**在golang中，使用sync.Cond可以实现条件变量。
- 无需通道携带数据，只要一个唤醒信号，理解成简化多轮go抢锁过程
...
# 应用
- 锁的使用，sync.Cond条件变量 channel context等
- 原子操作的使用 sync.WaitGroup sync.Once 

- <font color="red">Mutex 作为 sync 包的核心，支撑了 RWMutex，channel，singleflight 等多个并发控制的能力</font>
 - 对信号量的管理又是 Mutex 的基础, 虽然源码看不到，但 Golang 其实在扩展库 golang.org/x/sync/semaphore 也提供了一套信号量的实现(Weighted结构体也使用了sync.Mutex)
