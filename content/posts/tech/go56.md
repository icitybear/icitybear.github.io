...
# 缓存方面
1. go-cache 内存缓存 线程安全 遍历比sync.Map效率低  https://github.com/patrickmn/go-cache  
对于常规Web应用的本地缓存，go-Cache足够使用；

2. bigCache 对GC停顿敏感、海量条目、写入吞吐要 求极高 严格 FIFO/LRU  是更好的选择。https://github.com/allegro/bigcache 
  - https://juejin.cn/post/7107635176263385118 源码解析
3. ristretto LFU （TinyLFU）LRU 需要智能淘汰、高命中率的通用缓存 要极致命中率和智能管理 github.com/dgraph-io/ristretto
...
- <font color="red">spew 在生产里不要滥用，可能会导致 oom</font> 通过配置 spew.Config.MaxDepth 可以控制，但你不知道什么时候会有多层嵌套的大结构体
...
# json与结构体方面 
- mapstructure
{{< innerlink src="posts/tech/go29.md" >}}
- copier
{{< innerlink src="posts/tech/go61.md" >}}
...
- mergo 合并结构体字段的 https://darjun.github.io/2020/03/11/godailylib/mergo/ 业务配置使用
- <font color="red">sonic 字节封装的包 相比官方json包效率更高</font>
- flexjson包 自己封装的解决调用php返回数据的兼容问题
