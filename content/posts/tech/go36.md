---
title: "Go-fmt与log日志-log,slog,logrus,zap,zerolog" #标题
...
# <font color="red">slog</font>官方库
slog 是 Go 1.21 引入的官方结构化日志库（Structured Logging）。它结束了 Go 标准库只有简单 log 包的历史，让我们可以直接输出 JSON 或 Key-Value 格式的日志

- [slog使用入门](https://mp.weixin.qq.com/s/-Uq5SE5NHzcnYFH-HgxanA)
  - 同时输出控制台和日志文件 go1.26 版本后实现了slog.NewMultiHandler，1.26 前可使用io.Multiwriter
  - 轮转日志文件 不想用系统的 logrotate ，可以使用 lumberjack 包，它提供了更灵活的日志轮转策略。
  - slog 还支持自定义日志级别 (一般来说默认的日志级别已经够用了)
  
# <font color="red">logrus</font>
