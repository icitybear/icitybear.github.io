...
# group聚合时自定义新字段
1. 如果插入时使用查询的自定义字段会出现报错,为不同操作使用不同模型,一般复用结构体字段
2. Having时使用原始聚合表达式，因为FROM → WHERE → GROUP BY → HAVING → SELECT
``` go
type MediaCommonRecord struct {
	ID            int       `json:"id" gorm:"column:id"`                                  // 计划id
	CommonType    string    `json:"common_type" gorm:"column:common_type"`                // 业务类型 konghao, baoben yusuan,shauitui
	MediaCode     string    `json:"media_code" gorm:"column:media_code"`                  // 媒体渠道
	TaskID        int       `json:"task_id" gorm:"column:task_id"`                        // 任务id
	TaskName      string    `json:"task_name" gorm:"column:task_name"`                    // 任务名
	AdvertiserID  string    `json:"advertiser_id" gorm:"column:advertiser_id"`            // 广告主
	TaskType      int32     `json:"task_type" gorm:"column:task_type"`                    // 执行方式 1定时任务, 2立即执行
	ExecuteStatus int32     `json:"execute_status" gorm:"column:execute_status"`          // 1进行,2完成,3失败,4中断,5不执行
	ObjectType    string    `json:"object_type" gorm:"column:object_type"`                // 对象类型 advertiser project promotion
	ConditionJson string    `json:"condition_json" gorm:"column:condition_json"`          // 条件触发设定
	DealJson      string    `json:"deal_json" gorm:"column:deal_json"`                    // 处理设定
	FinishNum     int       `json:"finish_num" gorm:"column:finish_num"`                  // 执行成功数量
	Remark        string    `json:"remark" gorm:"column:remark"`                          // 任务崩溃信息
	CreateAdminID string    `json:"create_admin_id" gorm:"column:create_admin_id"`        // 创建账号id
	CreateTime    int       `json:"create_time" gorm:"column:create_time"`                // 创建时间
...
type MediaCommonRecordGH struct {
	MediaCommonRecord
	LogNum int `json:"log_num" gorm:"column:log_num"`
}

// RecordListWithTotal 获取列表(包含总条数)
func (r *MediaCommonRecordRepo) RecordListWithTotal(ctx context.Context, params *models.MediaCommonRecordSearchParam, currentPage int, pageSize int, sort *models.SortParam) ([]*models.MediaCommonRecordGH, int64, error) {
	var res []*models.MediaCommonRecordGH
	txDb := r.data.db.WithContext(ctx)

	// 产品要求不止成功数量 而是要日志数量 只能先连表处理
	txDb = txDb.Table("media_common_record t")
	txDb.Select("t.*,COUNT(t1.id) AS log_num")
	txDb.Joins("left join media_common_log t1 on t.id = t1.record_id")
	if len(params.IdList) != 0 {
		txDb = txDb.Where("t.id in(?)", params.IdList)
	}
	if params.CommonType != "" {
		txDb = txDb.Where("t.common_type = ?", params.CommonType)
	}
	if params.MediaCode != "" {
		txDb = txDb.Where("t.media_code = ?", params.MediaCode)
	}
	if params.TaskName != "" {
		txDb = txDb.Where("t.task_name like ?", "%"+params.TaskName+"%")
	}
	if len(params.TaskIdList) > 0 {
		txDb = txDb.Where("t.task_id in(?)", params.TaskIdList)
	}

	if len(params.ExecuteStatusList) > 0 {
		txDb = txDb.Where("t.execute_status in(?)", params.ExecuteStatusList)
	}
	if len(params.TaskTypeList) > 0 {
		txDb = txDb.Where("t.task_type in(?)", params.TaskTypeList)
	}

	// 时间范围
	if params.StartTimestamp > 0 {
		txDb = txDb.Where("t.create_time >= ?", params.StartTimestamp)
	}
	if params.EndTimestamp > 0 {
		txDb = txDb.Where("t.create_time <= ?", params.EndTimestamp)
	}
	txDb.Group("t.id")
	if len(params.FinishNumList) == 1 {
		if params.FinishNumList[0] == 1 {
			// txDb = txDb.Where("finish_num = 0")
			txDb.Having("COUNT(t1.id) = 0")
		} else {
			// txDb = txDb.Where("finish_num > 0")
			txDb.Having("COUNT(t1.id) > 0")
		}
	...
	var total int64
	err := txDb.Model(&models.MediaCommonRecord{}).Count(&total).Error
	if err != nil {
		return nil, 0, fmt.Errorf("[RecordListWithTotal] db count err:%v", err)
	}

	if sort != nil {
		txDb = txDb.Order("t." + sort.String())
	}
	...
	if err != nil {
		return res, total, fmt.Errorf("[RecordListWithTotal] db page err:%v", err)
	}
...
# 分表 gorm.io/sharding
目前有个bug 不同表有不同的sharding config
- [支持同一个数据库有多个表，不同的分片键](https://github.com/go-gorm/sharding/pull/170/files)
官方暂时未合并该代码,先自己替换
``` go
go.mod里
replace gorm.io/sharding => git.internal.taqu.cn/go-modules/gorm-sharding v0.6.2-0.1
```
![alt text](image.png)
...
# 字段设置为结构体
- 使用model的时候支持 字段为结构体指针类型 实现序列化和反序列化就行 Value和Scan
- 正常调用create和find等方法
- 只有使用Updates map[string]any时需要自己手动序列化成字符串
``` go
// MtxAgentProjectDiscountJson
type MtxAgentProject struct {
	ID               int                          `gorm:"column:id" json:"id"`
	MediaCode        string                       `gorm:"column:media_code" json:"mediaCode"`
	AgentCode        string                       `gorm:"column:agent_code" json:"agentCode"`
	OpenAgentCode    string                       `gorm:"column:open_agent_code" json:"openAgentCode"`
	AgentProjectCode string                       `gorm:"column:agent_project_code" json:"agentProjectCode"`
	AgentProjectName string                       `gorm:"column:agent_project_name" json:"agentProjectName"`
	DiscountJson     *MtxAgentProjectDiscountJson `gorm:"column:discount_json" json:"discountJson"` // 本质是string
	AgentType        string                       `gorm:"column:agent_type" json:"agentType"`
	CreateAdminId    string                       `gorm:"column:create_admin_id" json:"createAdminId"`
	UpdateAdminId    string                       `gorm:"column:update_admin_id" json:"updateAdminId"`
	CreateTime       int                          `gorm:"column:create_time" json:"createTime"`
	UpdateTime       int                          `gorm:"column:update_time" json:"updateTime"`
	DeleteTime       soft_delete.DeletedAt        `gorm:"column:delete_time" json:"deleteTime"`
	ModifyTime       time.Time                    `gorm:"column:modify_time;autoUpdateTime" json:"modifyTime"`
	Agent            *MtxAgent                    `gorm:"foreignKey:AgentCode;references:AgentCode" json:"agent"`
}

func (m *MtxAgentProject) TableName() string {
	return "mtx_agent_project"
}

//  进行序列化和反序列化 支持gorm
type MtxAgentProjectDiscountJson struct {
	DiscountRate MtxAgentProjectDiscountJsonDiscountRateArr `json:"discountRate,omitempty" mapstructure:"discountRate"` // 数组结构体类型
	HandlingFees MtxAgentProjectDiscountJsonHandlingFeesArr `json:"handlingFees,omitempty" mapstructure:"handlingFees"`
}

// Value 结构体序列化
func (p *MtxAgentProjectDiscountJson) Value() (driver.Value, error) {
	return json.Marshal(p)
}

// Scan  值反序列化
func (p *MtxAgentProjectDiscountJson) Scan(d any) error {
	v, ok := d.([]byte)
	if !ok {
		return nil
	}
...
// Bind 解析输入值到结构体 pb结构体转目标结构体
func (p *MtxAgentProjectDiscountJson) Bind(d *pb.DiscountObj) *MtxAgentProjectDiscountJson {
	if len(d.DiscountRate) > 0 {
		for _, item := range d.DiscountRate {
			p.DiscountRate = append(p.DiscountRate, &MtxAgentProjectDiscountJsonDiscountRate{
	...
	if len(d.HandlingFees) > 0 {
		for _, item := range d.HandlingFees {
			p.HandlingFees = append(p.HandlingFees, &MtxAgentProjectDiscountJsonHandlingFees{
				Gradient1:          item.Gradient1,
				Gradient2:          item.Gradient2,
				Gradient2Condition: item.Gradient2Condition,
				St:                 item.St,
				Et:                 item.Et,
				AgentCode:          item.AgentCode,
			})
...
// Unbind 解析结构体到输入值 目标结构体转pb结构体转
func (p *MtxAgentProjectDiscountJson) Unbind() (*pb.DiscountObj, []string) {
	pbDiscount := &pb.DiscountObj{}
	var agentCodeArr []string

	if len(p.DiscountRate) > 0 {
		sort.Sort(sort.Reverse(p.DiscountRate)) // 逆序排序 st降序
		for _, item := range p.DiscountRate {
			pbDiscount.DiscountRate = append(pbDiscount.DiscountRate, &pb.DiscountObjDiscountRateObj{
				DiscountRate: item.DiscountRate,
				St:           item.St,
				Et:           item.Et,
				AgentCode:    item.AgentCode,
			...
			agentCodeArr = append(agentCodeArr, item.AgentCode)
		}
	}

	if len(p.HandlingFees) > 0 {
		sort.Sort(sort.Reverse(p.HandlingFees)) // 逆序排序 st降序
		for _, item := range p.HandlingFees {
			pbDiscount.HandlingFees = append(pbDiscount.HandlingFees, &pb.DiscountObjHandlingFeesObj{
				Gradient1:          item.Gradient1,
				Gradient2:          item.Gradient2,
				Gradient2Condition: item.Gradient2Condition,
				St:                 item.St,
	...
	return pbDiscount, agentCodeArr
}

// NewestDF 获取最新的折扣率或者费率，优先费率 返回最后一个数值
func (p *MtxAgentProjectDiscountJson) NewestDF() (*MtxAgentProjectDiscountJsonDiscountRate, *MtxAgentProjectDiscountJsonHandlingFees) {
	// 排序实现了 less st升序 返回最后一个元素（最新的）
	if len(p.HandlingFees) > 0 {
		sort.Sort(p.HandlingFees)
		return nil, p.HandlingFees[len(p.HandlingFees)-1]
	}

	if len(p.DiscountRate) > 0 {
		sort.Sort(p.DiscountRate)
		return p.DiscountRate[len(p.DiscountRate)-1], nil
	}
...
type MtxAgentProjectDiscountJsonDiscountRateArr []*MtxAgentProjectDiscountJsonDiscountRate

func (m MtxAgentProjectDiscountJsonDiscountRateArr) Len() int {
	return len(m)
}

// Less 排序规则按照st升序
func (m MtxAgentProjectDiscountJsonDiscountRateArr) Less(i, j int) bool {
	return m[i].St < m[j].St
}

func (m MtxAgentProjectDiscountJsonDiscountRateArr) Swap(i, j int) {
	m[i], m[j] = m[j], m[i]
}

type MtxAgentProjectDiscountJsonDiscountRate struct {
	DiscountRate float32 `json:"discountRate" mapstructure:"discountRate"`
	St           string  `json:"st" mapstructure:"st"`
	Et           string  `json:"et,omitempty" mapstructure:"et"`
	AgentCode    string  `json:"agentCode" mapstructure:"agentCode"`
}

type MtxAgentProjectDiscountJsonHandlingFeesArr []*MtxAgentProjectDiscountJsonHandlingFees

func (m MtxAgentProjectDiscountJsonHandlingFeesArr) Len() int {
	return len(m)
}

// Less 排序规则按照st升序
func (m MtxAgentProjectDiscountJsonHandlingFeesArr) Less(i, j int) bool {
	return m[i].St < m[j].St
}

func (m MtxAgentProjectDiscountJsonHandlingFeesArr) Swap(i, j int) {
	m[i], m[j] = m[j], m[i]
}

type MtxAgentProjectDiscountJsonHandlingFees struct {
	Gradient1          float32 `json:"gradient1" mapstructure:"gradient1"`
	Gradient2          float32 `json:"gradient2" mapstructure:"gradient2"`
	Gradient2Condition float32 `json:"gradient2Condition" mapstructure:"gradient2Condition"`
	St                 string  `json:"st,omitempty" mapstructure:"st"`
	Et                 string  `json:"et,omitempty" mapstructure:"et"`
	AgentCode          string  `json:"agentCode" mapstructure:"agentCode"`
}

			discountModel := &models.MtxAgentProjectDiscountJson{}
			_, _ = discountModel.Bind(pbObj).NewestDF() // st升序
			discountByte, err := discountModel.Value()  // 存放序列化的值 因为update操作 所以要直接序列化成字符串
```
