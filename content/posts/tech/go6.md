...
- 指针则是用来<font color="red">指向这些变量值所在**内存地址的值十六进制表示(0x)**</font>,指针变量ptr自己有也有内存地址
- 变量值所在具体内存地址(也可能是存储指针类型的值,也叫**指针地址**)，该内存地址存储的变量值，是指针指向的变量的具体值。
- <font color="red">一个变量是**指针类型**的</font>，那么就可以用这个变量(指针变量)来存储指针类型的值（该类型变量xxx的内存地址）
- 不能进行偏移和运算
- 声明指针后，未初始化默认值为 nil,<font color="red">初始化通过&获取变量内存地址赋值（new返回类型指针）</font>
- 通过 := 对指针类型进行声明并初始化,底层会自动判断指针的类型
- 使用场景：类型指针和切片类型里的数组指针(array unsafe.Pointer 指向存放数据的数组指针)
- 使用指针一定要保证并发安全,像 int、bool 这样的小数据类型没必要使用指针,指针最好不要嵌套

``` go  
a := 100
var ptr *int  // 声明指针类型 默认值为 nil
ptr = &a      // 初始化指针类型值为变量a的内存地址 &取地址操作
fmt.Println(ptr) // 0xc0000a2000
fmt.Println(*ptr) // 100 *取值操作 指针指向变量a的值
...
- 放指针变量前, * 表示一个指针变量所指向的存储单元（*ptr）,也就是指针变量指向的原变量的值（取指针指向内存地址的存的变量值）
- <font color="red">* 取值操作赋值操作符的左边时，表示指针指向的变量</font>,赋值操作符的右边时，取b指针的值（取值操作，就是关联指向变量x的值）
...
func swap(a, b *int)  {
    ...
    t := *a
    // 取b指针的值（赋值操作符的右边）, 赋给a指针指向的变量(放在赋值操作符的左边)
    *a = *b
    // 将a指针的值(t) 赋给b指针指向的变量
    *b = t
...
1. <font color="red">任何类型的指针都可以被转化为 unsafe.Pointe</font>
2. unsafe.Pointer 可以被转化为任何类型的指针
   
3. uintptr 可以被转化为 unsafe.Pointer
4. <font color="red">unsafe.Pointer 可以被转化为 uintptr </font>

**为什么要单独列出这个类型uintptr?**
<font color="red">uuintptr 是 Go 内置的可用于存储指针的整型，而整型是可以进行数学运算的,绕过 Go 指针的安全限制，实现对指针的动态偏移和计算</font>
...
``` go
arr := [3]int{1, 2, 3}
ap := &arr // 将数组 arr 的内存地址赋值给指针 ap

// 通过 unsafe.Pointer 这个桥梁转化为 uintptr 类型加上数组元素偏移量（通过 unsafe.Sizeof 函数获取）
tmp := uintptr(unsafe.Pointer(ap)) + unsafe.Sizeof(arr[0]) // 该数组第二个元素的内存地址
sp := (*int)(unsafe.Pointer(tmp)) // 转化为 int 类型指针
*sp += 3 // 指针指向的变量 进行值+3
