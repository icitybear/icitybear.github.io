...
- 通过range获取数组的值 -> 不能修改原数组中结构体的值： 只是值副本 只能通过下标
...
- 基于数组，做了一层封装
- 可变长度的、同一类型元素集合，切片的长度可以随着元素数量的增长而增长（但不会随着元素数量的减少而减少）
- 创建 基于数组、切片和make直接创建，本质都是基于数组
...
- <font color="red">使用 make() 函数生成的切片一定发生了内存分配操作，但给定开始与结束位置（包括切片复位）的切片（slice [开始位置 : 结束位置]）只是将新的切片结构指向已经分配好的内存区域，设定开始与结束位置，不会发生内存分配操作。</font>
