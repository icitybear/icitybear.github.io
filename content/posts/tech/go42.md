...
3. 暴露一个可导出的单例获取方法 GetXXX()，用于<font color="red">返回这个单例对象</font>
   - 规范的处理方式是，在不可导出单例类 singleton 的基础上包一层接口 interface，将其作为对导出方法 GetInstance 的返回参数类
- 
``` go
package singleton
...
- <font color="red">sync.Once本质就是封装了加锁 double check 模式， 且使用了原子操作</font>
  - <font color="red">sync.Once.Do 方法，并在方法入参传入一个需要保证全局只执行一次的闭包任务函数 f func()</font>
