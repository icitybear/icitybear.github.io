...
2. optOption 函数类型 返回同一个结构：Demo （结构体指针）, 各个自定义函数返回optOption函数（闭包），该方法用来修结构体字段
...
func NewConfig (opts ...optOption) *Config {
    conf := new(Config)
    for _, opt : = range opts {
        opt(conf) // opt函数类型是optOption 匿名的闭包，持有外部的Config（所以参数是类型指针）
    }
    return conf
}

// 第一种方式：直接把Option定义为一个函数
type optOption func(*User)
// 所以使用这种闭包 返回的类型是 optOption
func WithName (name string) optOption {
    return func (u *User) {
        u.Name = name
    }
}

func WithAge (age int) optOption {
    return func (u *User) {
        u.Age = age
    }
}

// cf := NewConfig(WithName("张三"), WithAge(10))
```

# <font color="red">optOption为interface</font>

- **<font color="red">配合适配器模式 参数为结构体指针 构造时修改字段,比第一种复杂，但是接口可以包含多个函数</font>**
...
3. **<font color="red">自定义匿名闭包函数赋值给函数变量，然后通过该optOption接口类型适配器（实现该接口方法），注入该函数变量</font>**
   - 函数类型作为适配器（函数类型也是可以实现接口），然后调用闭包函数（闭包函数持有外部demo的引用）
...
// 函数类型作为接口 相当于适配器（非正常模式，适用单个功能函数的适配器）
// 定义函数实直接实现接口 适配器模式
type Adapter func(*Demo) // tag: 参数为要修改的结构体类型指针
...
// client调用方  使用适配器模式修改成员
// NewDemo(NewCustom(aa,11), Adapter(xxxOpt))

```
- 具体案例: UserAae和UserName都实现了InterfaceOption这个接口（方法Apply(*User), 可更改参数）
``` go
...
// User构造器里 遍历的是 接口的方法
func NewUser (opts ...InterfaceOption) *User {
    user := new(User)
    for _ opt := range opts {
...
func (un *UserName) Apply(user *user) { //UserName实现了接口InterfaceOption
    user.Name = un.Name // 
}
...
func (ua UserAge) Apply(user *user){ //UserAge实现了接口InterfaceOption
...
func constructUser () {
    ...
    user2 := NewUser(NewUserName("大乔乔"), UserAge(ageOpt)) // http.HandlerFunc()就是这种模式处理
    fmt.Printf("%+v\n", *user2)
}
...
func (w Where) Apply(users []*User) []*User {
    ...
    for _, v : = range users {
        if v.Name == w.Name && v.Age >= w.FromAge && v.Age <= w.ToAge {
            rect = append(rect, v)
        }
...
func NewTqMarketingService(ctx context.Context, opts ...Option) xxx {
...
1. 需要更改 API 以纳入所有这些功能请求(需求扩张)， 本来就一个公共api
...
## 使用配置结构
配置结构可以随着新选项的添加而随着时间的推移而增长，而用于创建服务器本身的公共 API 保持不变。
   ...
   2. 大多数时候，API 的用户会期望使用其默认行为。即使他们不打算更改任何配置参数，这些调用者仍然需要为第二个参数传递一些内容。<font color="red">为什么 API 的用户需要构造一个空值，只是为了满足函数的配置结构</font>
  	...
  	![alt text](image4.png)
   3. <font color="red">努力确保 nil 永远不会成为需要传递给任何公共函数的参数</font>，解决方案：更改该NewServer函数以<font color="red">接受可变数量的参数。</font>NewServer现在只接受Config值，而不接受指向配置值的指针，删除nil可能的参数，并确保调用者无法保留对服务器内部配置的引用。
	![alt text](image5.png)
...
- <font color="red">缺点：使用建造者模式链式创建过程中，只允许返回一个参数一旦过程中发生错误，为了确保链式调用继续，即使需要错误处理也只能内聚的具体一个个方法内部，并不能将 err 传递出来。从而只能在 Builder 方法内验证可能发生的错误，使得 err 的处理成本大大提高。</font>
