...
- 单机锁也是用原子cas进行操作的
# 使用go关键字 
- <font color="red">注意闭包问题, 引入外部变量，导致闭包。</font>协程有竞争关系，导致共享变量，无法保证原子性（导致数据丢失）。 <font color="red">没有保护共享资源 会有竞态条件（数据竞争）</font> 比如引入单机锁解决
- 注意子协程执行顺序状态问题
- <font color="red">起协程无返回值, 除非自己又进行封装,如errgroup包。每次起协程要考虑panic的捕捉问题，不然会外抛导致main主协程挂掉</font>
``` go
package main
​
import "fmt"
...
- 案例二：切片
切片自动扩容后，地址也会发生改变。一般解决方案，通过设置成员属性，扩容后也返回最新的，且变更时使用锁保护共享资源
``` go
func main() {
...
```
- 按照常规理解，上面的代码最终 returnList 长度应该是 1000 * 100 = 10万。但实际上我执行了10来次，最终长度都在1万多而已
