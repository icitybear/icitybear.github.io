---
title: "2.0-go并发编程-协程,协程间通信(基于共享内存)CSP,计数器和sync.WaitGroup,锁,原子操作" #标题
...
CSP模型的核心思想是通过消息传递来实现进程之间的通信。在CSP中，进程通过发送和接收消息进行交互，而不是通过共享内存来进行数据共享。这种方式可以避免并发访问共享数据时可能出现的竞态条件和死锁等问题。
...
```
- 协程，引入了不确定性, 不知道子协程什么时候执行完毕，运行到了什么状态 （会引导channel）
- 并发执行的，执行的先后顺序无法保证, 编排这段并发执行代码的执行顺序，<font color="red">**让多个协程按照指定顺序依次执行，需要依赖协程间的通信**</font>
- 为了避免这个问题，可以在循环体内创建一个新的变量，将当前循环变量的值赋给它，然后在闭包中使用这个新变量。这样每个闭包就会捕获到不同的值。
...
**Go 底层封装的一个更加优雅的计数器实现(sync.WaitGroup)，和自行实现的计数器一样，本<font color="red">质上都是通过计数信号量基于共享内存实现协程之间的同步和通信</font>**
