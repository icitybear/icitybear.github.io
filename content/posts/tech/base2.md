...
- 并行
  - **从整个操作系统层面来说，多个任务是可以并发执行的，因为 CPU 本身通常是多核的(并行)，而且即使是单核 CPU，也可以通过时间分片的方式在多个进程/线程之间切换执行（并行）**
...
- Go 语言的协程系统是基于「消息传递系统」实现的, <font color="red">不要通过共享内存来通信，而应该通过通信来共享内存</font>
