...
- 协程是用户态下二次加工的产物，需要引入额外的复杂度，但是相对于线程而言有着更轻的粒度和更小的开销 <font color="red">协程间的切换管理不依赖于**系统的线程和进程**，也不依赖于 CPU 的核心数量</font>
  - 传统的就是协程绑在线程上去执行，如果有同一个线上下的多个协程，如果第一个堵塞了，后面的也要等待 GM模型 比如PHP的扩展卡Swoole
  - go的GMP模型 goroutine 共享内存
