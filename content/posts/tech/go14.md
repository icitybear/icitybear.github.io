...
- go提供了一个 reflect 包用于提供反射相关的 API
    ...
    - reflect.Type 变量的类型 reflect.TypeOf函数获取。
    - reflect.Value 存储任何类型的值，reflect.ValueOf 函数获取
    - 获取结构体信息后dogValue
      - dogValue.Type().Field(i) 获取属性值
      - ogValue.Type().Field(i).Name  属性名
      - ogValue.Type().Field(i).Type 属性类型
      - dogValue.Type().Method(i).Call([]reflect.Value{}) <font color="red">调用该方法</font>
      - ogValue.Type().Method(i).Name  获取方法名
      - ogValue.Type().Method(i).Type 获取方法类型
- 通过反射获取变量的所有未知结构信息，以结构体为例（基本类型只有类型和值，更加简单），包括其属性、成员方法的名称和类型，值和可见性，还可以动态修改属性值以及调用成员方法。
...
# 空结构体
- struct{} 没有任何属性和成员方法的空结构体, 该类型的实例值只有一个，那就是 struct{}{}, 这个值在 Go 程序中永远只会存一份，并且占据的内存空间是 0
