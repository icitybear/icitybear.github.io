...
- Parse默认UTC时区, ParseInLocation可指定时区
- 第二参数为要转换的时间字符串，跟模板格式一样
- 使用time包的<font color="red">Parse函数来判断指定的日期字符串是否是一个正确的时间</font> 如果正确就无err == nil, 正确转为time.Time瞬时时间
``` go
// 默认UTC时区
...
// 对应输出
2020-01-01 10:00:00 +0800 CST

2020-01-01 00:00:00 +0000 UTC
2020-01-01 08:00:00 +0800 CST
```

## 定时器 time/tick.go文件
- 多次定时：time.Ticker 类型代表一个间隔触发的时间事件，它会以一个间隔 Duration（例如，1 秒、1 分钟等）触发多次，发送一个事件给一个通道C。使用time.NewTicker函数创建
``` go
type Ticker struct {
...
```
- 一次性定时：time.Timer 类型表示单次时间事件。当定时器到期时，当前时间会被发送给 C，使用time.NewTimer函数创建
